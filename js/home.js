...
let assistiu = ['movie_25', 'movie_26', 'movie_27', 'movie_28', 'movie_29', 'movie_30', 'movie_31', 'movie_32', 'movie_33'];
...
function imgCarrouselJPEG(arrayImgs, elementHtml) {
  for (let i = 0; i < arrayImgs.length; i++) {
    ...
    img.src = `./assets/img/movies/${arrayImgs[i]}.jpg`;
...
function carrouseOriginalNetflix(arrayImgs, elementHtml) {
  for (let i = 0; i < arrayImgs.length; i++) {
    ...
    item.classList.add('item');
    let img = document.createElement('img');
    img.classList.add('box-movie-original');
    img.src = `./assets/img/movies/${arrayImgs[i]}.jpg`;
    item.appendChild(img);
    elementHtml.appendChild(item);
  }
...
imgCarrouselJPEG(assistiu, pqAssistiu);
